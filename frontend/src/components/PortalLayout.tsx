...
import type { BrandSettings, UserPublic } from "@/lib/types";
import { Activity, ClipboardCheck, Database, FileUp, FolderKanban, LayoutDashboard, LogOut, Search, Settings, Users } from "lucide-react";
...
const nav = [
  ...
  ["/approvals", "Approvals", ClipboardCheck, "approvals-nav-link"],
] as const;

export default function PortalLayout() {
  const { data: user } = useQuery({ queryKey: ["me"], queryFn: () => apiGet<UserPublic>("/auth/me"), staleTime: 60_000 });
  const { data: brand } = useQuery({ queryKey: ["branding"], queryFn: () => apiGet<BrandSettings>("/branding"), staleTime: 60_000 });
  const visibleNav = nav.filter(([href]) => user?.role === "admin" ? href === "/approvals" : user?.role === "user" ? href === "/dashboard" || href === "/search" : true);
  return (
    <div data-testid="portal-shell" className="min-h-svh bg-[#020617] text-slate-200">
      <header data-testid="portal-header" className="sticky top-0 z-30 border-b border-slate-800/90 bg-slate-950/90 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-[1500px] items-center gap-4 px-4 sm:px-6">
          <Link to={user?.role === "admin" ? "/approvals" : "/dashboard"} data-testid="brand-home-link" className="group flex min-w-fit items-center gap-3">
            <span data-testid="brand-mark" className="grid size-9 place-items-center overflow-hidden rounded-xl bg-cyan-400 text-slate-950 shadow-[0_0_24px_rgba(34,211,238,.22)]">{brand?.logo_data_url ? <img data-testid="brand-logo-image" src={brand.logo_data_url} alt="Company logo" className="size-full object-cover" /> : <Database className="size-5" />}</span>
            <span data-testid="brand-name" className="hidden max-w-44 truncate text-sm font-bold tracking-tight text-slate-100 sm:block">{brand?.company_name ?? "Company Database"}</span>
          </Link>
          <nav data-testid="primary-navigation" className="ml-2 hidden items-center gap-1 overflow-x-auto md:flex">
            {visibleNav.map(([href, label, Icon, testId]) => (
              <NavLink key={href} to={href} data-testid={testId} className={({ isActive }) => `flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-medium transition-colors ${isActive ? "bg-cyan-400/10 text-cyan-300" : "text-slate-400 hover:bg-slate-800 hover:text-slate-100"}`}>
                <Icon className="size-4" /><span>{brand?.nav_labels[href.slice(1)] ?? label}</span>
              </NavLink>
            ))}
          </nav>
        ...
        <div data-testid="mobile-navigation" className="flex gap-1 overflow-x-auto border-t border-slate-900 px-3 py-2 md:hidden">
          {visibleNav.map(([href, label, Icon, testId]) => <NavLink key={href} to={href} data-testid={`mobile-${testId}`} className={({ isActive }) => `flex min-w-fit items-center gap-1 rounded-md px-3 py-1.5 text-[11px] ${isActive ? "bg-cyan-400/10 text-cyan-300" : "text-slate-400"}`}><Icon className="size-3.5" />{brand?.nav_labels[href.slice(1)] ?? label}</NavLink>)}
        </div>
      </header>
...
export function Panel({ children, className = "", ...props }: ComponentProps<"section"> & { children: ReactNode }) { return <section className={`rounded-2xl border border-slate-800 bg-slate-900/70 p-5 shadow-2xl shadow-slate-950/20 ${className}`} {...props}>{children}</section>; }